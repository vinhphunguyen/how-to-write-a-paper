...
\item \texttt{Skim}: is an open-source PDF reader for macOS. Alternatives are \texttt{Sumatra PDF} for Windows and \texttt{evince} for Linux;
...
It is not a requirement that the font used in figures match that of the text. Yet, it would be better if they match. \cref{fig:cold-spray-plot} is an example of a figure with non-matching text. The \LaTeX\ code used to include it in this paper is shown in \cref{snippet_latex_figure}. And the \texttt{Matplotlib} based \texttt{Python} script used to produce the PDF image with non-matching font is given in \cref{snippet_matplotlib} (\cref{sec:scripts}).
However, if you really want the font in your plots exactly match that in the text, see \cref{fig:cold-spray-plot-pgf} for such an example, the script in \cref{snippet_matplotlib_pgf} (\cref{sec:scripts}) can be used.
...
Another type of images in scientific papers is contour plots (see \cref{fig:figures1}), which are the outcomes of some visualization applications such as \texttt{ParaView}. We do not know how to match the font used in these images with the one in the text. We simply save them as PNG files of biggest resolution and include them in the TEX document in the same manner as PDF images.
 ...
 Pay attention to the figure captions. Ideally, the reader should be able to ascertain the entire story just by reading the figure captions i.e. without going back and forth between the figure and the text sections.
...
Making tables with footnotes in \LaTeX\ is not straightforward.  \cref{table:params-foot} is one example and we refer to the source to see how it was made. It is probably not an elegant solution but it is the best solution we have found.

\begin{table}
      \centering
      \caption{A table with footnotes generated using packages \texttt{enumitem,threeparttablex}.}
      \setlength\fboxsep{0pt}
      \vskip-\topsep%
      \smallskip%
      \renewcommand\arraystretch{1.4}
      ...
      Parameter                & Section 5.1     & Section 5.2     \\
      \midrule
      Young's modulus [MPa]    & $210\times10^3$ & 145\tnotex{tnote:robots-r1}            \\
      Poisson's ratio [-]      & 0.3             & 0.45            \\
      Tensile strength [MPa]   & 2445            & 20\tnotex{tnote:robots-r2}              \\
          \midrule 
      Experimentally validated & n/a             & n/a            \\
      Solver                   & multi-step AM   & single-step AM implicit-explicit \\
      State                    & Plane strain    & Plane strain    \\
      ...
      \begin{tablenotes}
      \item\label{tnote:robots-r1}footnote1.
      \item\label{tnote:robots-r2}Taken from \cite{Mandal:EFM2019}.
    \end{tablenotes}
    \end{threeparttable}
