...
Publishing original research in a peer-reviewed and indexed journal is an important milestone for a scientist. It is an important parameter to assess academic achievements. However, technical and language barriers may prevent many enthusiasts from ever publishing. To ease the writing process, books and articles on how to write scientific papers have been written \citep{day1998write,ashby2000write}.

Building on the advices found in the literature and our total 20 years of experiences, this brief paper gives guidance in writing a paper about your research. Most of the advice applies equally to writing a thesis. Due to our background in computational mechanics -- a sub-branch of computational engineering and sciences-- we have used, as a model, a typical Materials project: one combining experiment with modeling and computation to explain some aspect of material behaviour. 

Different from existing related works, see e.g. \cite{day1998write,ashby2000write}, we focus on the tools or softwares that ease our writing process (see \cref{sec:tools}). These softwares are mostly open source and cross platform. \cref{sec:writing-tips} provides actionable suggestions on how to structure your paper, avoid common mistakes.  Particularly, we present an iterative writing process that is intertwined with your research.
As we use \LaTeX, a high-quality free typesetting system which is the de facto standard for the communication and publication of scientific documents, some \LaTeX\ related suggestions on how to prepare high-quality vector images, good-looking tables are provided (see \cref{sec:latex}).
...
If you use \texttt{Illustrator} for some drawings and need to include mathematical symbols in them, then try the application called \texttt{LaTeXiT}\footnote{This app can be found at \url{https://www.chachatelier.fr/latexit/}.} to typeset whatever symbols and drag and drop them to \texttt{Illustrator} as embedded PDFs\footnote{Presentations can be created using \texttt{Keynotes} with equations created using \texttt{LaTeXiT} in exactly the same way.}.  \cref{fig:figures11} presents an example. The same thing can be done using \texttt{Inkscape} with some \LaTeX\ extensions.
...
\begin{figure}[!h]
  \centering
\includegraphics{Taylor-bar-setup}
  \caption{Using \texttt{Illustrator} and \texttt{LaTeXiT} to produce vector images with \LaTeX\ symbols. The font in the figure is slightly different from the one in the text ($\nabla\cdot\boldsymbol{\sigma} = \boldsymbol{\mathit{0}}$).}
  \label{fig:figures11}
\end{figure}
...
Avoid overly complex notations. \cref{tab:complex-notation} gives some examples. Try to introduce a nomenclature in the paper, to ease things with the reviewers and readers.  Sometimes, a table such as the one \cref{tab:notations} does a good job to introduce main notations. 
...
Preparing two-column papers is harder than one-column papers. \cref{snippet_two_cols} presents \LaTeX\ snippets for long figures, tables and equations that span the whole width of the paper. That all \LaTeX\ can do for you. For equations that are just a bit longer than one column, you have to manually modify them to make them fit.
